...
# noise=rnorm(Ngene*2,0,1/16)

co=.03
# norm=              ; 1   :cor~.56;  2.5 : cor~.82
# unif= .03 : cor~.30; 1.4 :cor~.5;  3   : cor~.80

AB1.list = lapply(1:n,function(i){
  #cbind(runif(Ngene,0,1), runif(Ngene,0,1))
  z=runif(1,0,1) # note: keep sample independent!!
  AB.1=replicate(Ngene,co*z+runif(1,0,1)) # let random intercepts of biomarkers correlated
  AB.2=runif(Ngene,0,1) 
  cbind(AB.1,AB.2)
}) # random effects of each gene

# ab.1=lapply(1:n,function(ii){
#   z=runif(1,0,1)
#   replicate(Ngene,co*z+runif(1,0,1))
# })
# AB.1=do.call(rbind,ab.1)
# cor(AB.1)
...
names(data)=c("ID","item","years","AB.1","AB.2")
...
# noise=rnorm(5,0,1/16)
# cor=c(1,1,-1,-2,1)
# for (i in 1:Ngene){
#   nrw=length(which(sel[[i]]==TRUE))
#   value[sel[[i]]] = data[sel[[i]],'AB.1'] + data[sel[[i]],'AB.2']*data[sel[[i]],'years'] + cor[i]*rnorm(nrw,0,sig[i]) + noise[i]
# }

value = data[,'AB.1'] + data[,'AB.2']*data[,'years'] + rnorm(nrow(data),0,sig)
data = cbind(data,value)

response=lapply(1:Ngene,function(ii){value[sel[[ii]]]})
resp=do.call(cbind,response)
corv=cor(resp)
round(corv,1)
...
# g4=co*z1+noise[4]
# g5=co*z1+noise[5]
#AB.1=rbind(g1[i]+rnorm(1,0,1/16),g2[i]+rnorm(1,0,1/16),g3[i]+rnorm(1,0,1/16),g4[i]+rnorm(1,0,1/16),g5[i]+rnorm(1,0,1/16))
#AB.2=rbind(g1_slp[i],g2_slp[i],g3_slp[i],g4_slp[i],g5_slp[i]) #1.
