...
arma::field<arma::vec> vec_to_field(const arma::vec& mu,
    ...
    arma::field<arma::vec> mu_f(p_z_vec.size());

    int start = 0;
    for(int k=0; k<p_z_vec.size(); k++){
...
struct VBJM_para_t{
    ...
    VBJM_para_t(const List& paralist)
    {
        ...
        for(int i=0; i<n; i++){
            ...
            arma::uvec lower_indices = arma::trimatl_ind(size(Ltmp));
...
// update varitional parameters mu_i and V_i //
class updateMuVFun{
public:
    ...
    double EvaluateWithGradient(const arma::mat& muV, arma::mat& g)
    {
        ...
        int start = 0;
        for(int k=0; k<data.p_z_vec.size(); k++){
...
public:
    ...
    double EvaluateWithGradient(const arma::mat& para_all, arma::mat& g)
    {
        ...
        grad_field(3) = arma::sum(grad_weib,1);
        ...
        for(int k=0;k<data.K;k++){
            ...
            for(int j=0; j<D1h_alpha(k).n_elem; j++){
                D1h_alpha(k)(j) += arma::as_scalar(
                    data.Z_t(i,k).row(j) * VZa(arma::span(start,end), j)
                );
            }
...
//' @return a list
// [[Rcpp::export]]
List VBJM(const List& datalist, const List& paralist, int maxiter = 100,
          double eps=1e-4){
    ...
    for(iter=0; iter < maxiter; iter++){
        ...
        for(int i=0; i < data.n; i++){
            ...
            arma::vec muV(Lvec.size() + mu.size());
        ...
        if(iter > 1){
            ...
            double err_para = std::sqrt(arma::accu(
                arma::square(para_all-para_all_prev)));
            if(err_ELBO < eps or err_para<eps){
                break;
            }
            Rcout << "err_ELBO="<< err_ELBO << "; err_para=" << err_para <<"\n";
        }
